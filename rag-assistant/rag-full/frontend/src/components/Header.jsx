      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#6b7280' }}>
          <span style={{
            ...
            background: state.groqApiKey ? '#22c55e' : '#f59e0b',
        ...
        {/* Auto-speak toggle */}
        <button
          onClick={() => dispatch({ type: 'SET_AUTO_SPEAK', payload: !state.autoSpeak })}
          title={state.autoSpeak ? 'Auto-speak ON — click to turn off' : 'Auto-speak OFF — click to turn on'}
          style={{
            padding: '5px 10px', borderRadius: 8, fontSize: 12, cursor: 'pointer',
            border: '1px solid #e5e7eb',
            background: state.autoSpeak ? '#eef2ff' : '#fff',
            color:      state.autoSpeak ? '#6366f1' : '#6b7280',
            display: 'flex', alignItems: 'center', gap: 4,
          }}>
          {state.autoSpeak ? '🔊' : '🔇'}
          <span style={{ fontSize: 11 }}>{state.autoSpeak ? 'On' : 'Off'}</span>
        </button>

        {/* Dashboard */}
        <button onClick={() => dispatch({ type: 'SET_DASHBOARD', payload: true })}
          style={{ padding: '5px 12px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', fontSize: 12, cursor: 'pointer', color: '#374151' }}>
          📊 Dashboard
        </button>
